...
type Runner struct {
	...
	mu        Mutex
	available muBool
	closed    muBool // runner is closed
...
func (b *Runner) SetLoggerOutput(debug Writer) {
	...
	for {
		...
		case <-b.chStopSignal:
			...
			b.Logger.KeyBool("SET Runner.Available", false)
			{
				...
				b.Logger.String(buf.String()) // this can also be done by buf.Dump(Writer)
				buf.ReturnBuffer()
			}
...
func newRunnerStats() runnerStats {
	return runnerStats{
		Rejected:  NewMutexInt(),
		Accepted:  NewMutexInt(),
		Cancelled: NewMutexInt(),
		Completed: NewMutexInt(),
	}
}
